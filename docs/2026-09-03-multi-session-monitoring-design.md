...
### Phase 2 (고도화) — 대기
- [x] ~~**개입**: 관전 중 관리 에이전트에 사이드 프롬프트 주입~~ — **안 함(사장님 결정 2026-09-14)**.
      관제는 읽기전용으로 확정. 이유: 불가침 원칙(claude 상태 변경 금지)과 정면 충돌, 동시 입력 시
      대화 트리 분기·진행 중 턴 오염, 관리 에이전트가 모르는 하위 변화, 원격 경로의 인증 등급.
      대안 = 기존 경로(세션을 터미널 탭으로 열어 직접 입력)로 충분.
...
## Phase 2-3. 타임라인 필터 (v0.8.4 PWA, 2026-09-12)

**목표**: 세션 3~5개의 타임라인은 도구 행이 대부분이라 "호출선만 / 대화만 / 오류만" 또는 "이 에이전트만
잠시 빼고" 보고 싶다. 서버 변경 0.

**설계**
- 행은 이미 DOM 스트림 → 재렌더 대신 **표식 + 클래스 토글**. `renderEvent` 가 행에 `data-sid` 와
  `has-call`(calls_out)·`has-text`(text)·`has-tool`(tools)·`has-err`(tool_results.is_error) 를 달고 렌더
  시점에 `.fhid` 를 판정(증분 행도 즉시 필터 적용). 필터 변경은 `bodyW.children` 한 번 순회.
- 필터 바(범례 아래): 종류 단일 선택(전체/→ 호출선/💬 대화/⚠ 오류) + 세션별 👁 토글(숨김은 취소선·흐림)
  + 카운터 `표시 N / 전체 M`(필터 없으면 `전체 M`). 범례가 다시 그려질 때(추가/제거/색 확정) 필터 바도 갱신.
- 필터는 **표시만** 거른다: 자동 스크롤(nearBottom)·우측 뷰어·자동 추적·활동점은 그대로(숨긴 세션의
  활동도 뷰어/점으로는 보임 — 놓치지 않게).
- 상태는 오버레이 수명 동안만(저장 안 함) — 저장 그룹의 기본 필터는 수요 확인 후.

**검증**: 릴레이 앱에서 저장 그룹 048f244c 스냅샷 46행으로 종류별 카운트가 클래스와 1:1, 세션 숨김/복원,
카운터 문구, 콘솔 오류 0.

## Phase 2-4. 관제 오버레이 안 💾 저장/갱신 (v0.8.5 PWA, 2026-09-12)

**목표**: 알림의 근거인 "저장 그룹"을 피커로 돌아가지 않고 관전 중에 만들거나(ad hoc 그룹), 관전 중
세션을 더하고 뺀 결과를 같은 그룹에 반영한다. 서버 변경 0(`POST /api/owner/monitor/groups` 의 `id` 교체 재사용).

**설계**
- `openMonitor(ids, manager, labels, opts)` 에 `opts.gid/name` — 저장 그룹으로 열면(피커 '열기'·알림 딥링크·
  피커 "💾 저장하고 관전") 💾 는 갱신 모드. 헤더 💾(`.mon-save`): 미저장 → 이름 모달(기본값 관리 라벨) → 저장
  → 강조색. 저장 상태에서 `group` add/remove 메시지가 오면 `.dirty`(주황 점) — 클릭하면 같은 id 로 교체.
- 구성 = 오버레이의 현재 `group` 맵(관리가 제거됐으면 첫 세션을 관리로, 라벨은 서버 확정 세션 라벨).
- 변경 표시를 글자(`💾*`)로 하면 34px 버튼에서 줄바꿈 — CSS `::after` 점으로(툴팁이 설명).

**검증**: 릴레이 앱에서 ad hoc 저장 → Host 목록 생성(알림 기본값) → 저장 그룹 재개 → 세션 추가 → 1초 내 점
→ 갱신 → subs 반영, 재로드 후 재현, 콘솔 오류 0.
