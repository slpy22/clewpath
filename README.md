...
| 터미널 탭 | 여러 세션 터미널을 탭으로 열어두고 클릭으로 전환하며 작업 (배경 탭의 claude 는 PC 에서 계속 실행) |
...
**여러 터미널을 오가며**: 터미널 화면 위 탭 스트립의 **`+ 탭`** 으로 다른 세션을 함께
열어두고 클릭으로 전환한다. 화면에 안 보이는 배경 탭의 claude 도 PC 에서 계속 돈다.
탭의 **×** 는 "화면만 닫기"(계속 실행, 나중에 다시 열면 이어짐) / "세션 종료"(프로세스까지
끝냄)를 고른다. 열어둔 탭은 다음에 앱을 열 때 목록으로 복원된다. 동시에 열 수 있는
터미널 수는 PC 쪽 `SM_MAX_TERMINALS`(기본 8) 로 제한된다 — 탭 하나가 claude 프로세스
하나다.
